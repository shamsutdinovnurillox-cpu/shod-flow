...
const KIND_STYLES: Record<string, string> = {
  ...
  Document: "bg-surface-2 text-muted",
};
    ...
    <div ref={boxRef} className="relative w-full max-w-md">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-border bg-surface-2 py-2 pl-9 pr-9 text-sm focus:border-blue-500 focus:bg-surface focus:outline-none"
        />
        {loading && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 animate-spin text-faint" />}
      </div>

      {open && (
        <div className="absolute z-50 mt-2 w-full max-h-96 overflow-y-auto rounded-lg border border-border bg-surface shadow-xl">
          {hits.length === 0 ? (
            ...
            <ul className="divide-y divide-border">
              {hits.map((h) => (
                <li key={`${h.kind}-${h.id}`}>
                  <button onClick={() => go(h)} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-surface-2">
                    <span className={`rounded px-2 py-0.5 text-xs font-semibold ${KIND_STYLES[h.kind] ?? "bg-surface-2 text-muted"}`}>{h.kind}</span>
